...
usage(void)
{
	fprintf(stderr, "usage: %s [-6dDGnNOpPruX][-b baud][-c ctl]* [-g gw]"
		"[-h host][-m mtu]\n"
...
parse6pref(int argc, char **argv)
{
	...
	case 1:
		if (parseip(conf.v6pref, argv[0]) == -1)
			error(1, 0, "bad address %s", argv[0]);
		break;
	}
...
parse6ra(int argc, char **argv)
{
	...
	if (conf.maxraint < conf.minraint)
		error(1, 0, "maxraint %d < minraint %d",
...
parseargs(int argc, char **argv)
{
	...
	if (argc > 0){
		...
		case Vtree:
		case Vpkt:
			error(1, 0, "medium %s already specified", conf.type);
		case Vadd:
		case Vremove:
		case Vunbind:
		case Vaddpref6:
...
main(int argc, char **argv)
{
	...
	case 'G':
		...
		break;
	case 'h':
		snprintf(conf.hostname, sizeof conf.hostname, "%s",
			 EARGF(usage()));
		sendhostname = 1;
		break;
	case 'm':
...
doadd(int retry)
{
	...
	if (ipv6auto) {
		if (ip6cfg(ipv6auto) < 0)
			error(1, 0, "can't automatically start IPv6 on %s",
				conf.dev);
//		return;
	} else if (validip(conf.laddr) && !isv4(conf.laddr)) {
		if (ip6cfg(0) < 0)
			error(1, 0, "can't start IPv6 on %s, address %i",
				conf.dev, conf.laddr);
//		return;
	}
	...
	if(!validip(conf.laddr))
		if(retry && dodhcp && !noconfig){
			...
			error(1, 0, "no success with DHCP");

	if(!noconfig)
		if(ip4cfg() < 0)
			error(1, 0, "can't start ip");
...
doremove(void)
{
	...
	if(!validip(conf.laddr))
		error(1, 0, "remove requires an address");
...
lookforip(char *net)
{
	...
	if (stat(proto, &s) < 0)
		error(1, 0, "no ip stack bound onto %s", net);
}
...
binddevice(void)
{
	...
	if(myifc < 0){
		...
		if(conf.cfd < 0)
			error(1, 0, "opening %s/ipifc/clone: %r", conf.mpoint);
...
dhcptimer(void)
{
	...
	switch(conf.state) {
	default:
		error(1, 0, "dhcptimer: unknown state %d", conf.state);
	case Sinit:
	case Sbound:
		break;
	case Sselecting:
...
openlisten(void)
{
	...
	for (n = 0; (cfd = announce9(data, devdir, 0)) < 0; n++) {
		if(!noconfig)
			error(1, 0, "can't announce for dhcp: %r");
	...
	if(write(cfd, "headers", strlen("headers")) < 0)
		error(1, 0, "can't set header mode: %r");

	sprintf(data, "%s/data", devdir);
	fd = open(data, O_RDWR);
	if(fd < 0)
		error(1, 0, "open %s: %r", data);
	close(cfd);
	return fd;
}
...
ndbconfig(void)
{
	...
	db = ndbopen(0);
	if(db == NULL)
		error(1, 0, "can't open ndb: %r");
	if (strcmp(conf.type, "ether") != 0 && strcmp(conf.type, "gbe") != 0 ||
	    myetheraddr(conf.hwa, conf.dev) != 0)
		error(1, 0, "can't read hardware address");
	...
	ndbfree(t);
	if(!validip(conf.laddr))
		error(1, 0, "address not found in ndb");
}
...
optgetx(uint8_t *p, uint8_t opt)
{
	...
	case Taddrs:
		...
		for(i = 1; i < n; i++){
			asprintf(&ns, "%s %s=%i", s, o->name, &ips[i*IPaddrlen]);
			free(s);
			s = ns;
		}
		break;
